...
fprintf('Imported MOSAiC FYI dataset.\n')
fprintf('Processed %d ice cores.\n', ...
    height(Tavg))
fprintf('Output saved to:\n%s\n', ...
    outputFile)

dataset = repmat("MOSAiC FYI",height(Tavg),1);
core = string(Tavg.DateTime);
date = Tavg.DateTime;
ice_age = repmat("FYI",height(Tavg),1);
